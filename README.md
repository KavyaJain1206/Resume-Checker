...
A replica of the _Playbook Diagnostic_ platform: a frictionless student data-intake
...
production (systemd + gunicorn) setup; the shipped systemd unit runs
`alembic upgrade head` before each start so VPS pulls pick up new tables.
